...
RSS_FALLBACK: Final[Template] = Template(
    ...
    "<rss version=\"2.0\">"
    "<channel>"
        "<title>{{ site.title|default(\"Feed\") }}</title>"
        "<link>{{ site.url|default(\"https://example.com\") }}</link>"
        "<description>{{ site.description|default(\"Newest Pages\") }}</description>"
        "<lastBuildDate>{{ now }}</lastBuildDate>"
        "{% for page in tree %}"
        "<item>"
            "<title>"
                "{{ page.title|forceescape }}"
            "</title>"
            "<description>"
                "{% if page.cover_image %}"
